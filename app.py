...
def index():
    try:
        conn = get_db_connection()
        if not conn: return "<h3>⚠️ فشل الاتصال بقاعدة البيانات - تحقق من إعدادات Render</h3>", 500
        ...
        <style>
            ...
            {% endif %}
            <h4>📍 الصفقات المفتوحة ({{ ot|length }})</h4>
            <table>
                <tr><th>العملة</th><th>الحالي</th><th>الربح ($)</th></tr>
                {% for t in ot %}
                {% set p = ((t.current_price - t.entry_price) / t.entry_price) * 50 %}
                <tr><td><b>{{ t.symbol }}</b></td><td style="color:#f0b90b;">{{ t.current_price }}</td><td class="{{ 'up' if p >= 0 else 'down' }}">${{ "%.2f"|format(p) }}</td></tr>
                {% endfor %}
            </table>
        </body></html>
        """, inv=invested, un=unused, net=net, ot=ot)
    except Exception as e:
        return f"<h3>حدث خطأ داخلي: {e}</h3>", 500
    ...
    try:
        ...
        if conn:
            ...
            if trades:
                ...
                for s in symbols:
                    if s in tickers:
                        close_position(s, tickers[s]['last'], "👤 إغلاق كلي")
    except Exception as e:
