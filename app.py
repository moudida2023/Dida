...
TRAILING_TRIGGER = 0.02    # يبدأ التتبع عند 2%
TRAILING_CALLBACK = 0.01   # يغلق عند هبوط 1% من القمة

portfolio = {"open_trades": {}}
closed_trades_history = []

# ======================== 2. إدارة تليجرام التفاعلية ========================
...
def handle_telegram_commands():
    ...
    main_menu = {
        "keyboard": [
            [{"text": "📊 حالة المحفظة"}, {"text": "📑 تقرير الساعة"}],
            [{"text": "🚨 إغلاق فوري (Panic)"}]
        ],
                ...
                for update in res["result"]:
                    ...
                    if "message" in update and "text" in update["message"]:
                        ...
                        if msg in ["/start", "العودة"]:
                            send_telegram_msg("🤖 نظام التداول الذكي جاهز. اختر أمراً:", main_menu)
                        
                        elif msg == "📊 حالة المحفظة":
                            txt = f"💰 *الرصيد:* `{VIRTUAL_BALANCE:.2f}$`\n🔄 *الصفقات:* `{len(portfolio['open_trades'])}`"
                            send_telegram_msg(txt)

                        elif msg == "📑 تقرير الساعة":
                            report = "📋 *الصفقات النشطة:*\n"
                            if not portfolio["open_trades"]: report += "_لا يوجد_"
                            for s in portfolio["open_trades"]: report += f"- `{s}`\n"
                            ...
                            send_telegram_msg(f"⚠️ تم تصفية `{count}` صفقات بنجاح.")
        except: time.sleep(5)
        time.sleep(1)

# ======================== 3. المحرك الفني (Analysis) ========================
...
async def get_score(sym):
    try:
        ...
        score = 0
        if l1h['close'] > l1h['ema21']: score += 30  # اتجاه ساعة صاعد
        if l15m['ema9'] > l15m['ema21']: score += 30 # تقاطع دخول
        if l15m['vol'] > l15m['vol_avg'] * 1.7: score += 20 # سيولة ضخمة
        if l15m['cmf'] > 0.1: score += 20 # تجميع شرائي
        
        return score, l15m['close']
    except: return 0, 0

# ======================== 4. إدارة التداول (Execution) ========================
            ...
            for sym in list(portfolio["open_trades"].keys()):
                ...
                if t['trailing_active']:
                    if (t['highest_price'] - cp) / t['highest_price'] >= TRAILING_CALLBACK:
                        ...
                        send_telegram_msg(f"💰 *جني أرباح (Trailing):* `{sym}`\nالربح: `{pnl:.2f}$`")
                        closed_trades_history.append({"sym": sym, "pnl": pnl})
                        portfolio["open_trades"].pop(sym)
                elif profit <= -0.04: # وقف خسارة ثابت
    ...
    try:
        ...
        for s in sorted(syms, key=lambda x: tk[x]['quoteVolume'], reverse=True)[:40]:
            ...
            if sc >= MIN_SCORE_TO_ENTRY:
                ...
                VIRTUAL_BALANCE -= BASE_TRADE_USD
                send_telegram_msg(f"🟢 *فتح صفقة:* `{s}`\nالسكور: `{sc}/100`")
...
async def main():
    send_telegram_msg("🚀 **البوت النهائي جاهز للعمل!**\nاستخدم الأزرار للتحكم.")
