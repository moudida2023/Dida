...
TRAILING_CALLBACK = 0.005  # إغلاق عند تراجع 0.5% من القمة

portfolio = {"open_trades": {}}
closed_trades_history = []

# ======================== 2. برمجة أوامر تليجرام ========================
            ...
            if "result" in response:
                for update in response["result"]:
                    ...
                    if "message" in update and "text" in update["message"]:
                        ...
                        if text == "/start":
                            msg = "🟢 **بوت القناص يعمل الآن!**\nاستخدم الأوامر: /status, /report, /panic, /close"
                            send_telegram_msg(msg)

                        elif text == "/status":
                            total_pnl = sum([t['pnl'] for t in closed_trades_history])
                            msg = (f"💰 **الحساب:**\n• الرصيد: `${VIRTUAL_BALANCE:.2f}`\n"
                                   f"📈 الأرباح: `${total_pnl:.2f}`\n"
                            ...
                            send_telegram_msg(f"🚨 تم إغلاق {count} صفقات يدوياً.")

                        elif text.startswith("/close"):
                            sym = text.replace("/close", "").strip().upper()
...
async def scan_market():
    ...
    try:
        ...
        for sym in sorted(symbols, key=lambda x: tickers[x]['quoteVolume'], reverse=True)[:25]:
            ...
            for sym in list(portfolio["open_trades"].keys()):
                ...
                if trade['trailing_active']:
                    if (trade['highest_price'] - cp) / trade['highest_price'] >= TRAILING_CALLBACK:
                        ...
                        portfolio["open_trades"].pop(sym)
                        send_telegram_msg(f"💰 **جني ربح:** `{sym}` | +${pnl:.2f}")
...
async def main_loop():
    send_telegram_msg("✅ **تم تشغيل النظام بنجاح!**")
